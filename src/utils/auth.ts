...
export const auth = betterAuth({
	...
	emailAndPassword: {
		...
		sendResetPassword: async ({ user, url, token }, request) => {
			...
			const newUrl = (process.env.NODE_ENV === 'development' ? process.env.FRONT_END_URL : process.env.FRONT_END_URL?.startsWith('http') ? process.env.FRONT_END_URL : `https://${process.env.FRONT_END_URL}`) + callbackURL! + '?token=' + token
